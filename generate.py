...
class GPUMemoryMonitor:
    def __init__(self, device_index=5, interval=0.1):
        self.device_index = device_index
        self.interval = interval  # 监控间隔（秒）
        self.peak_usage = 0  # 峰值显存占用（MB）
    ...
    def _monitor(self):
        ...
        while not self.stop_event.is_set():
            ...
            current_usage = info.used / (1024 ** 2)  # 转换为MB
...
outline_prompt = ("The answer format requirement is as follows:"
...
def normal_decode(model, tokenizer, prompt):
    full_prompt_with_template = tokenizer.apply_chat_template([{"role": "user", "content": prompt}],
                                                              add_generation_prompt=True, tokenize=False,thinking=False,
                                                              max_length=1000000)

    input_batch = tokenizer([full_prompt_with_template], padding=True, truncation=True, max_length=10000,
...
def parallel_decode(model, tokenizer, prompt):
    ...
    try:
        ...
        raise
        return {"response": "Parallel decoding failed", "time": time.time() - start_time, "length": 0, "num_para": 0,
                "stage1_response": ""}
    ...
    response_parallel = \
    tokenizer.batch_decode(output_parallel[:, input_batch["input_ids"].size(1):], skip_special_tokens=True)[0]

    stage1_response = \
    tokenizer.batch_decode(stage1_output[:, input_batch["input_ids"].size(1):], skip_special_tokens=True)[0]

    return {"response": response_parallel, "time": parallel_time,
            "length": len(output_parallel[0, input_batch["input_ids"].size(1):]), "num_para": num_para,
...
def prefill(model, tokenizer, prompt):
    ...
    start_time = time.time()
    output = model(input_ids=input_batch["input_ids"])
    # print("Prefill Time:", time.time() - start_time)

    return {"response": "", "time": time.time() - start_time}
...
if __name__ == '__main__':
    ...
    if task == "retrieval":
        df_path = "./datasets/student_resume_logic_retrieval/logic_gpa_resume_10.jsonl"
    ...
    for i in tqdm(range(len(df))):
        ...
        normal_response, normal_time, normal_length = normal_output["response"], normal_output["time"], normal_output[
            "length"]

        df.loc[i, "normal_response"] = normal_response
        df.loc[i, "normal_time"] = normal_time
        df.loc[i, "normal_length"] = normal_length
        df.loc[i, "normal_max_memory"] = monitor.peak_usage # 记录最大显存
        ...
        df.loc[i, "parallel_max_memory"] = monitor.peak_usage  # 记录最大显存

        print("normal time:", normal_time, " length:", normal_length)
        print("parallel time:", parallel_time, " length:", parallel_length, " num_para:", num_para)

        # 只进行prefill，用以计算decode部分的时间
        prefill_output = prefill(model, tokenizer, full_prompt)
        prefill_time = prefill_output["time"]
        df.loc[i, "prefill_time"] = prefill_time
        torch.cuda.empty_cache()  # 清理显存


    save_dir = "./results_v1/" + os.path.basename(model_path) + "/"
