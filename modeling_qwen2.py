...
class Qwen2ParaModel(Qwen2PreTrainedModel):
    ...
    def forward(
        ...
        if position_ids is None:
            ...
            if num_para is not None and para_begin_position is not None and num_para>1:
                past_key_values_length_true=(past_seen_tokens - para_begin_position)//num_para+para_begin_position
                position_ids = (torch.ones(inputs_embeds.shape[1],dtype=torch.long,device=inputs_embeds.device)*past_key_values_length_true).unsqueeze(0)
            else:
                position_ids = cache_position.unsqueeze(0)
        ...
        if num_para is not None and para_begin_position is not None and self.config._attn_implementation != "flash_attention_2" and num_para>1:
            ...
            parallel_part_true_len=(causal_mask.size(3)-para_begin_position)//num_para
            #把offset为0，num_para，2*num_para...parallel_part_true_len*num_para的token的对角线置为0
            row_indices = torch.arange(0, num_para, device=causal_mask.device)
            for i in range(parallel_part_true_len):
                col_indices = torch.arange(para_begin_position+i*num_para,para_begin_position+(i+1)*num_para, device=causal_mask.device)
                causal_mask[:,:,row_indices,col_indices]=0

            #将beam_finished中对应位置的mask的行全部置为-inf
            if beams_finished is not None:
                for beam_finished_id in beams_finished:
                    causal_mask[:,:,beam_finished_id,:]=min_value
    ...
    def _update_causal_mask(
        ...
        # When output attentions is True, sdpa implementation's forward method calls the eager implementation's forward
        if self.config._attn_implementation == "sdpa" and not using_static_cache and not output_attentions and num_para is None:
            if AttentionMaskConverter._ignore_causal_mask_sdpa(
                attention_mask,
                inputs_embeds=input_tensor,
                past_key_values_length=past_seen_tokens,
...
class Qwen2ForParaCausalLM(Qwen2PreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]
    ...
    def generate_with_parallel(self,
        input_ids,
        attention_mask = None,
        use_cache = None,
        eos_token_id = None,
        ...
        **kwargs
        ...
        #
        # tokenizer = AutoTokenizer.from_pretrained(self.config.name_or_path)#"/data/models/Qwen2.5-7B-Instruct",trust_remote_code=True)
        #确保batchsize只能为1
        ...
        logits_processor = CustomLogitsProcessor(para_end_token_id=para_end_token_id,
                                                 ellipsis_token_id=ellipsis_token_id,
        ...
        output_stage1=self.generate(input_ids=input_ids,
                                         logits_processor= LogitsProcessorList([logits_processor]),
                                         eos_token_id=para_end_token_id,
        ...
        #删除空列表，并删除最后2个token
        each_beam=[each_beam_i for each_beam_i in each_beam if each_beam_i]
        each_beam=[beam[:-2] for beam in each_beam]
        each_beam=[[para_begin_token_id]+beam for beam in each_beam]
        ...
        start_time=time.time()
        #调用并行生成函数
        generated_output_para=self.generate_parallel(
            input_ids=None, #只获取最后一个词
            attention_mask=None,
            use_cache=use_cache,
            bos_token_id=para_begin_token_id,
            eos_token_id=para_begin_token_id,
            max_length=parallel_decoding_max_length,
            para_begin_position=para_begin_position,
            num_para=num_para,
            past_key_values=past_key_values_prefix,
            each_beam_prefix=each_beam,
        ellipsis_token_id=ellipsis_token_id,
            half_ellipsis_token_id=half_ellipsis_token_id,
        colon_token_id=colon_token_id,
            **kwargs
        )
    ...
    def generate_parallel(self,
        ...
        **kwargs
        ...
        with torch.no_grad():
            ...
            # # 如果没有提供 attention_mask，则创建一个全1的mask
            ...
            for i in range(max_length):
                ...
                #如果上一个token是colon_token_id，则下一个token不可能是ellipsis_token_id，即logits设为-65504
                if i>0:
                    for beam_id in range(num_para):
                        if previous_token[0,beam_id]==colon_token_id:
                            logits[0,-num_para+beam_id,ellipsis_token_id]=-65504
                            logits[0,-num_para+beam_id,half_ellipsis_token_id]=-65504



                #对最后num_para个token进行选择
                next_token=get_next_token_from_logits(logits,num_para=num_para)


                #如果previous_token中某个位置为eos_token_id，则next_token中对应位置强制设为为eos_token_id
                if i>0:
                    if isinstance(eos_token_id, int):
                        next_token[previous_token == eos_token_id] = eos_token_id
                    elif isinstance(eos_token_id, list):
                        for eos_token_id_i in eos_token_id:
                            next_token[previous_token == eos_token_id_i] = eos_token_id_i

                if each_beam_prefix is not None:
                    #每个beam的前端强制设为each_beam_prefix中对应位置的值
                    for beam_id in range(num_para):
                        if i<len(each_beam_prefix[beam_id])-1:
                            next_token[0,beam_id]=each_beam_prefix[beam_id][i+1]


                # 将生成的词（num_para个）添加到序列中
                generated_sequence = torch.cat([generated_sequence, next_token], dim=1)

                # generated_sequence_decode0=tokenizer.decode(generated_sequence[0].tolist()[0::num_para])
                # generated_sequence_decode1=tokenizer.decode(generated_sequence[0].tolist()[1::num_para])
                # generated_sequence_decode9=tokenizer.decode(generated_sequence[0].tolist()[5::num_para])
                ...
                if attention_mask is not None:
                    attention_mask = torch.cat(
                        [attention_mask, torch.ones((1, num_para), dtype=torch.long, device=attention_mask.device)],
                        dim=1)
                ...
                # #如果next_token中包含eos_token_id，则获取对应的序列的位置
                # beams_finished=[]
                # for beam_id in range(num_para):
                #     if next_token[0,beam_id]==eos_token_id:
                #         beams_finished.append(beam_id)

                # 如果所有beam都已经生成结束标记，则停止生成
                if isinstance(eos_token_id, int):
                    ...
                    eos_mask_list=[]
                    for eos_token_id_i in eos_token_id:
                        eos_mask_list.append(next_token == eos_token_id_i)
...
def get_next_token_from_logits( logits, num_para=1,do_sample=False, temperature=1.0, top_k=None, top_p=None)->torch.Tensor:
