...
struct Mask {
    ...
    const int para_begin_position, num_para;
    const float alibi_slope;
    const bool Is_para;

    __forceinline__ __device__ Mask(const int max_seqlen_k, const int max_seqlen_q,
                                    const int window_size_left, const int window_size_right,
                                    const int para_begin_position, const int num_para,
                    ...
                    for (int i = 0; i < size<0, 0>(tensor); ++i) {
                        ...
                        for (int nj = 0; nj < size<1, 1>(tensor); ++nj) {
                            ...
                            for (int j = 0; j < size<1, 0>(tensor); ++j) {
                                ...
                                if (Is_para) {
                                    //当元素位置（row_idx,col_idx）满足 row_id 不等于 (col_idx-para_begin_position)%num_para，将其值设为负无穷。如果 col_idx<para_begin_position，则不变。
                                    if (col_idx >= para_begin_position) {
                                        if ((col_idx - para_begin_position) % num_para != row_idx) {
                                            tensor(make_coord(i, mi), make_coord(j, nj)) = -INFINITY;
                                        }
                                    }
                                }
